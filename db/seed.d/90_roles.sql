...
--
-- Nothing here resolves a permission from a preset name at request time. The
-- preset is only what these rows were seeded FROM, so an edit on /people
-- sticks and moving somebody between presets on its own changes nothing.
--
...
begin
  ...
  update nl.users set role = 'account_manager', responsibility =
  ...
  update nl.users set role = 'admin', responsibility =
    'Runs sales. Sets what everybody else may approve, and can answer for any account.'
  where id = 1;

  update nl.users set role = 'ops_manager', responsibility =
    'Owns the policies, the agents'' autonomy and everything that escaped them.'
  where id = 5;

  update nl.users set role = 'inside_sales', responsibility =
  ...
  update nl.users set role = 'planner', responsibility =
    'Plans the families we make here and decides what a short line does.'
  where id = 12;

  update nl.users set role = 'buyer', responsibility =
  ...
  -- The ops manager and the admin hold every dimension. This is the decision
  -- the brief asks for: seeing everything is a row somebody granted, not what
