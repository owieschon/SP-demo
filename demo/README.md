# demo/: the earlier demo tooling

This folder and `tools/` predate Northline. They build an invented world for
an earlier CRM app, in a Supabase project of its own. Northline does not use
them, except `tools/scan.mjs` (the name scan before every commit).
...
application reads it through the same views and functions it uses
everywhere else: item ledger lines carry negative quantities for sales, the
