...
export interface DeskCapabilities {
	...
	/** The documents work's per-draft attachment store (migration 0020). */
	rfqAttachments: boolean;
}
...
export async function readCapabilities(tx: Tx): Promise<DeskCapabilities> {
	const [row] = await tx.sql<{ atp: string | null; projection: string | null; attachments: string | null }>`
		select to_regprocedure('nl.available_to_promise(text,int,date)')::text as atp,
		       ...
		       to_regclass('nl.rfq_attachments')::text as attachments`;
	return {
		availableToPromise: row?.atp !== null,
		openLineProjection: row?.projection !== null,
		rfqAttachments: row?.attachments !== null
	};
}
...
// ---------------------------------------------------------------------------
// The parts the mail names
// ---------------------------------------------------------------------------

export interface ResolvedItems {
	lines: { itemNo: string; quantity: number; asWritten: string; description: string }[];
	/** Part numbers in the mail that are not in the catalog, or are blocked. */
	unresolved: string[];
}

/**
 * Turn the part numbers as written into catalog numbers, in one lookup.
 *
 * Buyers write part numbers with the dashes missing, in lower case, or with a
 * space where the catalog has nothing, so the match is on the number with
 * every separator stripped. An exact hit wins; a stripped form that fits two
 * catalog numbers is left unresolved, because guessing between two parts is
 * how the wrong stack ends up on a truck.
 */
export async function resolveItems(
	tx: Tx,
	budget: LookupBudget,
	asked: { itemNo: string | null; quantity: number | null }[]
): Promise<ResolvedItems> {
	const written = asked
		.map((line) => ({ raw: (line.itemNo ?? '').trim(), quantity: line.quantity ?? 1 }))
		.filter((line) => line.raw.length > 0);
	if (written.length === 0) return { lines: [], unresolved: [] };

	const keys = [...new Set(written.map((line) => line.raw.toUpperCase().replace(/[^A-Z0-9]/g, '')))];
	const rows = await budget.run<{ key: string; item_no: string; description: string; blocked: boolean }>(
		'resolve_items',
		{ written: written.map((w) => w.raw) },
		() =>
			tx.query(
				`select w.key, i.item_no, i.description, i.blocked
				 from (select value as key from jsonb_array_elements_text($1::jsonb)) w
				 join nl.items i
				   on upper(regexp_replace(i.item_no, '[^A-Za-z0-9]', '', 'g')) = w.key
				 order by w.key, i.item_no`,
				[JSON.stringify(keys)]
			)
	);

	const byKey = new Map<string, { item_no: string; description: string; blocked: boolean }[]>();
	for (const row of rows ?? []) {
		byKey.set(row.key, [...(byKey.get(row.key) ?? []), row]);
	}

	const lines: ResolvedItems['lines'] = [];
	const unresolved: string[] = [];
	for (const line of written) {
		const key = line.raw.toUpperCase().replace(/[^A-Z0-9]/g, '');
		const matches = (byKey.get(key) ?? []).filter((m) => !m.blocked);
		const exact = matches.find((m) => m.item_no === line.raw);
		const item = exact ?? (matches.length === 1 ? matches[0] : null);
		if (!item) {
			if (!unresolved.includes(line.raw)) unresolved.push(line.raw);
			continue;
		}
		// The same part twice in one mail is one line with the quantities added.
		const existing = lines.find((l) => l.itemNo === item.item_no);
		if (existing) {
