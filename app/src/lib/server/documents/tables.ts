...
/** A row that states a total somewhere in it. */
const TOTAL_ROW = /\b(sub-?total|grand total|order total|total)\b/i;
/** The word "total" standing where a part number should be. */
const TOTAL_LABEL = /^\s*(sub-?total|grand total|order total|total)\b/i;

/**
 * True when a row says something about how many or for how much. A row with
 * something in the part column and nothing in any of those columns is a note
 * somebody typed under the table, not a line of the request.
 */
function hasFigures(fields: (TableField | null)[], cells: DocCell[]): boolean {
	return fields.some(
		(field, i) =>
			field !== null && field !== 'item' && field !== 'description' && (cells[i]?.text ?? '') !== ''
	);
}
 ...
 * may be split over two rows, as a merged header often is ("Quantity" above
 * an empty cell), so a blank label borrows the text from the cell above it.
 *
...
export function findGridTable(
	...
	for (let h = 0; h < grid.length; h++) {
		...
		for (let r = h + 1; r < grid.length; r++) {
			...
			blanks = 0;
			const itemText = cells[itemColumn].text;

			// A totals row, whether the word sits in the part column or in a
			// blank one further along. Keeping the figure matters: validation
			// checks their own total against their own lines.
			const wholeRow = cells.map((c) => c.text).join(' ');
			if (TOTAL_LABEL.test(itemText) || (itemText === '' && TOTAL_ROW.test(wholeRow))) {
				statedTotal = largestNumber(cells) ?? statedTotal;
				continue;
			}
			if (itemText === '' || !hasFigures(fields, cells)) {
				// Nothing in the part column, or a part with no figures at all:
				// a note under the table. Two of those and the table has ended.
				blanks += 1;
				if (blanks >= 2) break;
				continue;
			}
...
export function findTextTables(lines: DocLine[], name: string): DocTable[] {
	...
	for (let h = 0; h < lines.length; h++) {
		...
		for (; r < lines.length; r++) {
			...
			// A totals line under the table: it says "total" and holds no
			// quantity, price or date of its own.
			if (TOTAL_LABEL.test(itemText) || (TOTAL_ROW.test(text) && !hasFigures(fields, cells))) {
				statedTotal = largestNumber(cells) ?? statedTotal;
				continue;
			}
			// In running text, a line that does not fit the table's shape is
			// where the table ended.
			if (itemText === '' || !hasFigures(fields, cells)) break;
