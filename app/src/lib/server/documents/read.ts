...
export interface ReadResult {
	...
	stored: StoredFile[];
	/** Every file that was not read, each with its reason, in plain English. */
	problems: string[];
}
 ...
 *
 * Every file is read, and every reason a file could not be read is collected
 * rather than thrown, so the page can list all of them at once instead of
 * making somebody upload four files to find four problems. The caller
 * decides what to do with them; the intake page refuses the whole submission,
 * because a quote built from three of the four files somebody sent is worse
 * than no quote.
 */
export async function readRequest(input: ReadRequest): Promise<ReadResult> {
	...
	for (const file of input.files) {
		...
		try {
			...
			// Byte for byte the same file as one already in this upload. Saying
			// so beats storing it twice and quoting everything twice.
			const already = seen.get(one.sha256);
			if (already) {
				problems.push(
					one.fileName === already
						? `${one.fileName} was attached twice. It only needs to be read once.`
						: `${one.fileName} is the same file as ${already}. Attach one of them.`
				);
				continue;
			}
	...
	return { documents, stored, problems };
}
