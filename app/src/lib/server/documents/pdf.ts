 ...
 *
 * PDF.js reports a gap between two pieces of text as a piece of its own
 * holding a single space, whose width is the width of the gap. Those are
 * dropped here and the gap is measured from the positions instead, which is
 * the only way to tell a word break from a column break: both arrive as one
 * space, and only their width says which is which.
 */
