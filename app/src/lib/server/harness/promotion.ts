...
import { AGENT_MAILBOX_KIND } from './trust.ts';
...
export async function principalFor(db: Db, actorId: number, agent: AgentId): Promise<number> {
	const mailboxKind = AGENT_MAILBOX_KIND[agent];
	if (!mailboxKind) throw new NotAPrincipal(agent);
	/*
	  Resolved from the mailbox the agent holds in its scope, the same way the
	  harness's wake decides which agent a mailbox is (wake.ts). The alternative
	  is to write the seeded email address in here, which breaks silently if
	  anybody renames it. `kind = 'agent'` is checked as well, so this can never
	  hand back a person's id even if the scope data were wrong.
	*/
	const [row] = await db.asUser(actorId, (tx) =>
		tx.sql<{ id: number }>`
			...
			where u.kind = 'agent' and mb.kind = ${mailboxKind}
			limit 1`
	);
	if (!row) throw new NotAPrincipal(agent);
